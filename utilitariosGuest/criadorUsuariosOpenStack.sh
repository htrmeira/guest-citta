...
echo -e  "Esta Ã a lista atual de roles. Atribua algum deles ao usuario"
cat "roles.txt"
echo "Digite abaixo o role para o usuario"
read user_role
rm "roles.txt"
...
sleep 3
keystone user-role-add --user=$usuario --role=$user_role --tenant=$usuario
sleep 3
...
if [ "$user_role" == "convidado" ]; then
   echo "$usuario : $DATA : "0"" >> /home/openstack/utilitariosGuest/dataCriacaoDeGuest.txt
fi
